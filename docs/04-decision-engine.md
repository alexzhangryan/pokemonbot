...
**Where the three stand, as of 2026-09-13.** All three are built and measured on the guard (`docs/pruning-guard.md`). A, rebuilt to this section's specification (D61), is the agent's provider. B was built as specified in `docs/specs/2026-08-29-learned-policy-provider.md`, fit to 84,321 human decisions, and rejected (D67): it recalls what strong humans played far better than A does (`docs/policy-prior.md`) and discards far more of the equilibrium's mass at the agent's `k = 10`, intervals apart. Imitation accuracy is not a proxy for the guard in this format, and any future provider is read on the guard before its accuracy is believed. C, mocked with a local 3B model, lost the guard to the shortlist it was handed (D69); a stronger backend is Alex's call. The one open result is the union of A and B: it loses to A at `k = 5`, and beats A at `k = 10`, 15 and 20 on positions the current agent plays into (D69). The union is therefore a budget question rather than a provider question, and `k` is a search-cost decision that the M8 payoff model changes, so it is re-opened after M8 against whichever model wins (`docs/specs/2026-09-13-engine-gate.md`). The guard's number also depends on whose play produced the positions (D69): any comparison says which position set it was measured on.
