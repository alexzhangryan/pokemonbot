...
**Redone at the local figure, 2026-09-13.** `docs/benchmarks.md` measured 2.13 ms per clone-plus-step on the development hardware, over JSON-RPC, against the 4.7 ms above. At that figure the full 156 × 156 node is 51.8 s, the pruned node with 20 particles and 5 buckets is 21.3 s on one core, and depth 2 with pruning on both plies and no particles is also 21.3 s on one core — inside the 45 second turn limit, and about 3 s across 8 cores if one simulator process per core scales. Conclusion 1 holds, now on one core. Conclusion 2 does not hold as written: depth 2 fits the turn clock on the stock simulator at one particle and one roll replicate; what a 100 times faster engine buys is depth 2 *with* particles and replicates (35 minutes per turn on the stock simulator) or depth 3. Conclusion 3 becomes about 21,000 steps per turn on one core. One constraint the arithmetic above omits: the 7 minute player clock is a total, so a 45 s turn every turn forfeits after nine turns and the per-turn ceiling is not a per-turn budget. The M8 gate is specified against these numbers in `docs/specs/2026-09-13-engine-gate.md` section 2 (D70).
