...
## D91. The opponent's kind of turn is pinned to what people play, their columns contain switches, and their only living Pokemon now acts — 2026-09-23, Claude Code

Context: Alex asked for play-analyse-improve cycles against the ladder, capped at five. The first analysis read the 101 rated games played on `regmc-mence` after D90 (50-51, Wilson 0.40-0.59, all reviewed; `scripts/ladder_analysis.py` is the table), and three things in the turn model were wrong in ways the traces show directly.

1. **The equilibrium expected a Protect the opponents did not play.** By turn bucket the model's columns put 68, 33, 43 and 50 percent of their mass on a line with a Protect in it and 34, 7, 11 and 4 percent on a double Protect; the opponents protected on 14, 10, 6 and 4 percent of turns and double-protected on none. A Protect costs nothing in a one-turn model, so the adversary always has one, and our best response to an opponent who is about to Protect is to not attack. This is `STATUS.md`'s open item 1 (the model has no tempo), seen from the columns rather than the rows.
2. **The columns never contained a switch.** The opponents switched on 29 percent of first turns and 3 to 10 percent afterwards; the model gave every switch zero, because `opponent_candidates` only ever built move columns, so a hit aimed at a slot was always priced as landing on what stood there.
3. **A side with only its second slot alive did nothing in the model.** `opponent_candidates` skipped an empty slot and placed the living slot's action at index 0, where `TurnModel._order` found no unit and dropped it. 160 of the 783 scored live decisions were such positions. In a fifth of the game the equilibrium was solved against an opponent who could not attack.

Decision, in three parts.

**1. The column player's kind marginals are pinned to a prior from the corpus.** `champions.search.kinds` names a joint action's *kind* -- which slots attack, protect, use Fake Out, or switch -- and `scripts/build_action_prior.py` counts the kinds of every decision of every player in the 945 M-C corpus replays (1,890 sides, 8,674 decisions) by turn bucket (1, 2, 3, 4+) into `data/policy/actionkinds.gen9championsvgc2026regmc.json`; `docs/action-kinds.md` is the table. Turn one: attack+attack 40%, attack+switch 18%, attack+fakeout 18%, attack+protect 12%, protect+protect 3%. `matrix.solve_constrained` solves the game in which the opponent draws its kind from the prior with probability `PRIOR_WEIGHT` and plays the worst column of that kind for us, and plays the plain adversary otherwise: one LP with a value variable per kind, the column strategy read off the duals so `x^T A y` is still the value. The prior is renormalised over the kinds the column set offers. Every solve of a live turn, the lead sweep, the two-ply agents and the coach go through `kinds.solve_columns`, and the trace's `candidates` event records what was pinned (`column_prior`). `PRIOR_WEIGHT` is 0.8 and hand-set: the adversarial residual keeps a Protect where one is plainly right, and the next cycle reads the implied kind mass against the realised one to move it. The prior is keyed by format and lent along `LINEAGE` like every fitted artifact.

**2. Opponent columns contain switches.** After the `k` move columns, `_switch_columns` adds up to `DEFAULT_SWITCH_COLUMNS` (8) in which one slot switches to a bench Pokemon and the partner plays its top-threat option, round robin over slots and bench. The bench is what has been seen plus the opponent's previewed, not yet seen Pokemon, which `protocol.state.annotate_unseen` adds at full health, flagged `unseen`, as many as the bring leaves unaccounted for; the agent does this on its search snapshot after the belief annotation and the coach does it on a copy of the recorded state, so both solve the same column set and the coach's ex-ante columns are no longer only the switch the opponent turned out to make. The payoff model already placed the incoming Pokemon (D86), so the column prices a hit aimed at the slot landing on the switch-in. The lead sweep takes two switch columns rather than eight, for a sixth more cost rather than two thirds.

**3. Columns keep their slot index.** A slot with nothing in it contributes a `kind: none` no-op, so the living slot's action sits where the payoff model looks for it.

Rationale, and what is and is not claimed. All three are defects with a measurement behind them, not tuning: a mass the model put on a kind of turn nobody played, a kind of turn it could not express, and a fifth of positions in which the opponent's last Pokemon was inert. Re-reviewing a live game under the new coach, the opponent's turn-one mixture went from a 73 percent double Protect to attack+switch 30%, Fake Out+Protect 21%, Protect+attack 20%, Fake Out+attack 18%. **None of this has played a rated game**, and the D71 arithmetic still holds. What the next batch is read for is narrower: the implied kind mass against the realised one by turn (`ladder_analysis.py`), whether luck on turn one (+4.5 points a decision against the model, the worst turn) comes down, and whether the coach's preferred lines on mistakes stop being `attack+protect` 41 times in 158.

Not done, and why: the replay side of the coach (`from_replay`) does not add unseen Pokemon, because `positions.Board` seeds each side's whole bring already and the calibration is a separate measurement; `scripts/calibrate_coach.py` should be re-run before its bands are trusted under the new solver. The belief agent still spends 0.1 s of a 45 s turn; the clock is the next lever and this entry does not touch it.

Consequences: `champions/search/kinds.py` (new), `champions/search/matrix.py` (`solve_constrained`), `champions/search/policy.py` (`opponent_candidates`, `_switch_columns`, `EMPTY_SLOT`, `DEFAULT_SWITCH_COLUMNS`), `champions/protocol/state.py` (`unseen_pokemon`, `annotate_unseen`), `champions/agents/oneply.py`, `twoply.py`, `adaptive.py`, `champions/search/lead.py` (`PREVIEW_SWITCH_COLUMNS`), `champions/coach/analyze.py` (`Models.kind_prior`), `champions/coach/decisions.py` (`from_trace(dex=)`), `scripts/review.py`, `scripts/build_action_prior.py` (new), `scripts/ladder_analysis.py` (new), `data/policy/actionkinds.gen9championsvgc2026regmc.json`, `docs/action-kinds.md`, `tests/test_kinds.py`. Every review written before this entry was solved with the plain equilibrium and the misaligned columns; `make review` re-solves.

## D92. The one-ply agents solve over every legal row; the two-ply agents keep the budget — 2026-09-23, Claude Code

Context: cycle 2 of the autonomous ladder cycles, the first batch played on D91 (seed 2, 12 games, 7-5). Under the D91 coach, which solves with every legal row, the coach's best line was outside the agent's twelve pruned rows in 38 of 90 scored decisions (seed 1: 42 of 93), and those decisions carried two thirds of the ex-ante loss (310 of 456 points; seed 1: 214 of 346) and 18 of the 26 mistakes and blunders (seed 1: 15 of 25). The missing lines are pivots and Protects -- `Fake Out + switch to Rillaboom`, `Parting Shot + Protect`, `Protect + switch to Rillaboom` -- which the heuristic ranks below attacks and the twelve-row budget then leaves out. The pruning guard had already measured the heuristic at twelve as dropping the unpruned equilibrium's mass on about a third of positions at a mean value loss of 0.03 (`docs/pruning-guard.md`); the budget existed for a clock the agent does not use. The `belief` agent spends 0.1 s of a 45 s turn (D91's reading of 113 games), and the payoff matrix over every legal row of 40 real batch-2 positions costs 0.72 s a turn on average and 1.56 s at worst (mean 72 rows, at most 138, against 24 to 32 columns).

Decision. `OnePlyAgent.row_budget` is None: the policy ranks the whole legal set and every row but the disqualified ones (friendly fire, a status move at our own partner) goes to the matrix. The heuristic's ordering is kept, because its top row is still the anytime proposal made before the payoff loop starts. `TwoPlyAgent` and everything under it (`AdaptiveAgent`, `AdaptiveBeliefAgent`) keep `DEFAULT_K`, since the second ply multiplies the row count and D89's escalation lived inside 3-10 s a turn at twelve. An explicit `k` still overrides. The trace's `candidates` event records `row_budget` (`"all"` or the number) beside `k`.

Rationale, and what is and is not claimed. The equilibrium over a superset of rows is never worse for us on the model's own terms, and the coach was already measuring the agent against that superset, so every "mistake" it labelled for a line the agent could not consider was a measurement of the budget rather than of the choice. What is not claimed: that the model values the recovered lines correctly. Pivots and Protects are exactly where a one-turn model is weakest (open item 1, tempo), and a wider row set gives model error more to exploit. The next batch is read for the ex-ante loss (which should fall by construction), for luck (which says whether the recovered lines are model error), and for the win rate, which at twelve games says nothing on its own. The preview sweep is unchanged at eight rows: 225 openings at 72 rows would cost seven times its budget.

Consequences: `champions/agents/oneply.py` (`row_budget`, the disqualified filter, `row_budget` on the trace), `champions/agents/twoply.py`, `tests/test_row_budget.py`, and `tests/test_oneply.py`, whose pruning test now asserts the opposite of what it did. `docs/pruning-guard.md` still describes the budgets the two-ply agents and the preview sweep use.

## D93. The payoff model reads a move's priority from the dex, and a forced mid-turn switch has no opponent action to score — 2026-09-23, Claude Code

Context: cycle 3 of the autonomous ladder cycles, the first batch on D92 (seed 3, 12 games, 6-6). The ex-ante loss fell as D92 predicted (20 and 12 points a game in wins and losses, from 36 and 41) and the coach's best line was in the agent's rows in 111 of 111 decisions, but luck per decision rose to +4.5 points, and one signal held in every batch played this year: on turns where the opponent's line contained a Protect, our realised outcome was 8 to 12 points below the model's expectation (+7.8 over 127 such decisions before D91, +9.9, +11.5 and +8.7 in the three batches since). Diffing the model's predicted state against the real next state on the worst of those turns showed the cause. In every one the model had our attack land on the Pokemon that had Protected -- Kingambit knocked out through its Protect, Slowbro at 20 percent behind its Protect.

The opponent's realised column is rebuilt by the coach from the log's observations (`decisions._their_action_from_observations`) as `{kind, move, target, label}`, with no `priority`, and `TurnModel._order` read priority off the described slot. So in every ex-post cell the opponent's Protect resolved at priority 0, after our attacks, and protected nothing. The agent's own rows and its live opponent columns carry the dex's priority, so the agent's play was right; what was wrong was every ex-post number the coach has produced since D79 on a turn where the opponent Protected -- the expected value, the ex-post loss, the luck, and `best_ex_post`. D90's reading that the 76-game losses were luck rather than mistakes leaned on those numbers.

A second artefact came out of the same breakdown: our forced switches after a faint (`pass + switch to X`) read as 8 to 20 points *luckier* than expected in every batch. The tracer records a faint's forced switch as a second decision at the same turn number, and the coach scored the opponent's action for that turn against our switch-in as if it were simultaneous, so their attacks -- which had already resolved -- were applied a second time.

Decision. `TurnModel._priority` takes a move's priority from the dex whatever the slot carries, for every caller. `from_trace` gives a second decision at the same turn number no `their_played`, so the coach scores no ex-post cell for a forced switch (its ex-ante loss is still scored). Batches 1 to 3 are re-reviewed under this before being read.

Rationale, and what is and is not claimed. Both are defects with a mechanism, not tuning, and both bias luck in a known direction, so the per-batch luck read before this entry -- including D92's "+4.5" -- is not comparable to the read after it. What is not claimed: that D92 is vindicated or not; the corrected read of batch 3 is in `STATUS.md`, and twelve games remain twelve games.

Consequences: `champions/search/payoff.py` (`_priority`), `champions/coach/decisions.py` (`from_trace`), tests in `test_payoff.py` and `test_coach.py`. Every review written before this entry mis-scores the opponent's Protect turns ex post; `make review` re-solves. `docs/coach-calibration.md` was measured with the defect and should be re-run before its bands are cited.

## D94. A measured offset by the kind of our line is subtracted from the rows before the solve; the first one is our attack beside a voluntary switch — 2026-09-23, Claude Code

Context: cycle 4 of the autonomous ladder cycles played a second batch on D92 (seed 4, 12 games, 5-7), for 24 games and 177 scored decisions on every legal row against 24 games and 146 decisions on the twelve-row budget (D91, seeds 1 and 2), all reviewed under the D93 coach. The records do not separate (11-13 against 12-12). The luck does, narrowly: +1.5 (se 1.4) a decision on D92 against -2.4 (se 1.3) on D91, and it sits on one kind of line. Our attack beside a voluntary switch read +11.1 points a decision over 17 decisions in both D92 batches alike (+11.8 and +11.1); every other kind read within noise (attack+attack -0.6 over 66, attack+fakeout -1.8 over 29, attack+protect +2.4 over 24). Under D91 that kind was played once in 146 decisions. Diffing the model's predicted state against the real one on those turns showed the same thing each time: the partner that stayed in was knocked out before it moved, where the model had it moving first or surviving -- a Ceruledge the belief had slower than Gholdengo, a Dragonite that had invested in Speed, a Golisopod hitting twice as hard as its assumed set. The partner that stayed in fainted on 59 percent of those turns against 30 percent of plain attacking turns. The agent chooses the pivot exactly when the model overrates the partner that stays in, which is the winner's curse D92 named, concentrated on the rows D92 made available.

`scripts/kind_luck.py` measured the same luck by kind on 60 corpus games with open team sheets (120 sides, 538 decisions), where the opponent's sets are known: attack+attack -0.2 (se 0.2, n=250), attack+protect +0.3 (n=79), attack+switch -1.6 (se 0.8, n=72), attack+fakeout -0.5 (n=51), protect+switch -0.8 (n=30). With known sets the one-turn model is calibrated ex post to within a point on every kind, and the pivot is if anything slightly undervalued. The +11 on the ladder is therefore the belief -- the opponent's spread, item and speed order -- and the selection on it, not the model's mechanics.

Decision. `champions.search.kinds.RowOffsets`: a per-format file, `data/policy/rowoffsets.<format>.json`, of win-probability points to subtract from every cell of a row of a given kind before the solve, applied by `OnePlyAgent._search` (recorded on the trace as `row_offsets`) and by the coach's ex-ante solve (`Models.row_offsets`), and left out of the ex-post cell so luck keeps measuring the model. The shipped file carries one entry, `attack+switch: 0.111`, with its provenance (17 decisions, se 0.05, seeds 3 and 4). The rule for an entry, written in the file: a kind's measured live luck exceeds twice its standard error over at least 15 decisions, re-measured every cycle with `scripts/ladder_analysis.py`. A shifted cell is clipped to [0, 1], because it is still a probability and the trace and the tests hold it to that.

Rationale, and what is and is not claimed. This is the narrowest correction the evidence supports: it changes the value of one kind of line by the amount that kind was measured to be wrong, in the information state the agent actually plays in, and it is undone by editing one file. It is not a fix of the belief, whose speed and spread errors are what the offset stands in for, and it is not free of the hazard it addresses: 17 decisions is a thin measurement and the offset may be too large or too small by half. Cycle 5 (seed 5) plays it and is read for how often the pivot is still chosen and what its luck is; the offset should shrink toward zero as the belief improves, and a cycle that finds it near zero removes it. What the corpus number says beyond this entry: the next lever after the clock is the belief's speed and spread inference, because with known sets the model's ex-post error is a point.

Consequences: `champions/search/kinds.py` (`RowOffsets`, `load_row_offsets`, `apply_row_offsets`), `champions/agents/oneply.py`, `champions/coach/analyze.py`, `scripts/kind_luck.py` (new), `data/policy/rowoffsets.gen9championsvgc2026regmc.json` (new), `tests/test_kinds.py`. The kind-widened row set built in cycle 4 (`policy.widen_by_kind`, `OnePlyAgent.extra_per_kind`) stays in the code, off, as the alternative if the offset does not hold.

## D95. The lead sweep blends in what people lead and bring with our six; the ledger records the public rating after every game; the corpus's teams are extracted as candidates — 2026-09-26, Claude Code

Context: Alex asked what would move the rating and then to do all of it. The account stands at Elo 1170, GXE 44.3, Glicko 1457 +- 26, 125-139 over 264 rated games, against a top-500 cutoff of 1564 on this ladder. The peak Elo could only be reconstructed (about 1280, 15 September) because `battle.rating` was None on every game the project played and Showdown publishes only the current number. And the lead: Gholdengo with Incineroar in 32 of the 60 cycle games at 15-17, where the 47 corpus games on the same six lead it 8 times and Rillaboom with Sneasler 7; the sweep's turn-one value did not separate wins from losses over 161 games.

Decision, in three parts.

**1. A corpus prior on leads and brings for the six we play.** `scripts/corpus_teams.py` groups every open-sheet side in the corpus by its six -- 433 distinct teams, 224 seen three or more times -- and writes `data/policy/teams.<format>.json` with each team's record, the most common set of every member, and the lead pairs and brings people chose. `champions.search.leadprior.load_team_prior` reads the entry for our six (five games or more, else no prior), and `lead_sweep` blends the corpus's lead shares, scaled so the most common pair is 1.0, with its own pair values at `LEAD_PRIOR_WEIGHT = 0.3`, and the bring shares by species into the choice of the back two the same way. The trace's `preview_decision` carries `lead_prior` and `blended_values` beside the raw `pair_values`. The weight is hand-set; the next cycles read the lead distribution against the corpus's and the turn-one win probability by lead.

**2. The ledger records the public rating.** `scripts/ladder_live.py` fetches `pokemonshowdown.com/users/<name>.json` three seconds after each game and writes `elo`, `gxe`, `glicko`, `glicko_dev`, `ladder_w` and `ladder_l` onto the row, so the peak is measured from here on rather than reconstructed. The site refuses Python's default agent string; the request names the project.

**3. The corpus's ten most-played teams are candidate team files**, `data/teams/corpus-NN-<anchor>.txt`, the most common set of each member with stat points by the belief's own rule for a nature (D85), since sheets never carry points. All ten validate under the format and play. `scripts/team_tournament.py` plays candidates against a pool with the ladder agent on both sides, one worker per local server, for the question STATUS asked: which team does *this* agent play best. A `belief-wide` agent (`WideBeliefAgent`, twelve rows plus two of each missing kind) is registered for the row-set A/B D94 called for.

What is not claimed: any of it has moved the rating. The tournament, the depth re-measurement (`adaptive-belief` against `belief`, 100 games) and the row-set A/B (`belief-wide` against `belief`, 100 games) are running as this is written; their readings and the choices they force are the next entry. The belief's speed and spread inference, which D94 named as where the remaining error lives, was inspected and not changed: `SpreadBelief.allocation` already maxes Speed for an attacker of base Speed 70 or more, and the failures on the pivot turns were not a rule that a night's work could correct with a measurement behind it.

Consequences: `champions/search/leadprior.py` (new), `champions/search/lead.py`, `champions/agents/oneply.py`, `champions/agents/belief_agent.py` (`WideBeliefAgent`), `scripts/ladder_live.py`, `scripts/selfplay.py`, `scripts/corpus_teams.py` (new), `scripts/team_tournament.py` (new), `data/policy/teams.gen9championsvgc2026regmc.json`, `data/teams/corpus-*.txt`, `tests/test_leadprior.py`.

## D96. The one-ply agents solve over the heuristic's twelve rows widened by the best two of each kind of turn they leave out — 2026-09-26, Claude Code

Context: D92 solved every legal row and D94 found the model's optimism concentrated on the rows that change made available; the corrected ladder read was luck -2.4 a decision on twelve rows (seeds 1 and 2, 146 decisions) against +1.5 and +3.9 on every row (seeds 3 to 5). D94 asked for an A/B on more games than a cycle can play. `WideBeliefAgent` -- twelve rows plus the best two rows of each kind of turn the twelve lack (`policy.widen_by_kind`, D92's flagged-off alternative; 14 rows a position on real positions, 24 at most, against 39 for every legal row) -- played `BeliefAgent` on every legal row for 100 mirror games on `regmc-mence` (`runs/mc-rows`, seed 200): **55-45 for the widened set** (Wilson 0.45-0.65).

Decision. `OnePlyAgent.row_budget = DEFAULT_K` and `extra_per_kind = 2` are the defaults, so `belief` plays the widened set; the two-ply agents keep the plain twelve. The trace's `row_budget` reads `12+2/kind`. `WideBeliefAgent` stays in the registry as the explicit name for what is now the default, and every legal row is one constructor argument away (`k=None`).
